---
name: stash
description: Search a separate local Agent Skills library or explicitly manage inactive standalone skills. Use only when the user explicitly invokes `/stash:stash` to open, find, list, install or update Stash, archive, activate, deactivate, or inspect a stored skill. Do not invoke Stash implicitly for ordinary work.
